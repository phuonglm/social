 ...
 * @author hoat_le
 */
public class SpaceActivityPublisherTest extends AbstractAPI1xTest {
  ...
  public void setUp() throws Exception {
    ...
    Assert.assertNotNull("activityManager must not be null", activityManager);
    identityManager =  (IdentityManager) getContainer().getComponentInstanceOfType(IdentityManager.class);
    Assert.assertNotNull("identityManager must not be null", identityManager);
    spaceService = (SpaceService) getContainer().getComponentInstanceOfType(SpaceService.class);
    Assert.assertNotNull("spaceService must not be null", spaceService);
    spaceActivityPublisher = (SpaceActivityPublisher) getContainer().getComponentInstanceOfType(SpaceActivityPublisher.class);
    Assert.assertNotNull("spaceActivityPublisher must not be null", spaceActivityPublisher);
  }
  ...
  public void testSpaceCreation() throws Exception {
    ...
    Assert.assertNotNull("space.getId() must not be null", space.getId());
    ...
    Assert.assertTrue(activities.get(0).getTitle().contains(space.getDisplayName()));
    Assert.assertTrue(activities.get(0).getTitle().contains("root"));

    ActivityStream activityStream = activities.get(0).getActivityStream();

    Assert.assertNotNull("activityStream.getId() must not be null", activityStream.getId());

    Assert.assertEquals("activityStream.getPrettyId() must return: " + space.getPrettyName(), space.getPrettyName(), activityStream.getPrettyId());
    Assert.assertEquals(ActivityStream.Type.SPACE, activityStream.getType());
